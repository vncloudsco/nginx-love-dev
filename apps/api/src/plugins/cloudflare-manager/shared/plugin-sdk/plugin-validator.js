...
class PluginValidator {
    ...
    validateMetadata(metadata) {
        ...
        else if (!semver_1.default.valid(metadata.version)) {
        ...
        if (!metadata.type || !Object.values(types_1.PluginType).includes(metadata.type)) {
            errors.push(`Plugin type must be one of: ${Object.values(types_1.PluginType).join(', ')}`);
        }
        ...
        if (metadata.minSystemVersion) {
            if (!semver_1.default.valid(metadata.minSystemVersion)) {
                errors.push('minSystemVersion must be a valid semver');
            }
            else if (semver_1.default.lt(this.systemVersion, metadata.minSystemVersion)) {
        ...
        if (metadata.maxSystemVersion) {
            if (!semver_1.default.valid(metadata.maxSystemVersion)) {
                errors.push('maxSystemVersion must be a valid semver');
            }
            else if (semver_1.default.gt(this.systemVersion, metadata.maxSystemVersion)) {
     ...
     */
    validatePackageStructure(files) {
        const errors = [];
        const warnings = [];
        ...
        const suspiciousFiles = files.filter(f => f.includes('..') ||
            f.startsWith('/') ||
